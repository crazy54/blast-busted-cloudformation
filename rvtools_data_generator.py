...
# --- Full CSV Headers (as defined in earlier successful steps) ---
CSV_HEADERS = {
...
# Ensure all defined CSVs are in the generation order
for key in CSV_HEADERS.keys():
    if key not in CSV_GENERATION_ORDER:
        CSV_GENERATION_ORDER.append(key) # Add any missing ones to the end

# --- SHARED_CONTEXT for Cross-CSV Consistency ---
SHARED_CONTEXT = {
    'dcs': {}, 'clusters': {}, 'hosts': {}, 'vms': {},
    'datastores': {}, 'networks': {},
    'os_names': list(set([ # Using set to ensure unique OS names initially
        "Microsoft Windows Server 2019 (64-bit)", "Microsoft Windows Server 2022 (64-bit)",
        "Ubuntu Linux (64-bit)", "Red Hat Enterprise Linux 8 (64-bit)",
        "VMware ESXi 7.0", "CentOS 7 (64-bit)", "Debian GNU/Linux 11 (64-bit)"
    ])),
}
REUSE_PROBABILITY = 0.6 # Chance to reuse an existing entity from SHARED_CONTEXT
MAX_NAME_GEN_ATTEMPTS = 50
DEBUG_ENTITY_GENERATION = False # Set to True for verbose entity generation logs
...
def generate_ai_data(prompt="Generic AI prompt", current_row_context=None, **kwargs):
    ...
    # For now, it returns a placeholder indicating what might be generated.
    # Placeholders in the prompt (e.g., [VM_NAME]) would be replaced using current_row_context.
    return f"AI_GEN_DATA_FOR[{prompt.split(' ')[0]}]" # Shortened for brevity
...
def _ensure_unique_name(name_prefix, collection_dict, max_val, attempts_limit, suffix_format="{:02d}", suffix_chaos=False):
    attempts = 0
    name_to_use = f"{name_prefix}{suffix_format.format(random.randint(1, max_val))}"
    if suffix_chaos: # For things like VM names where suffix isn't strictly sequential
         name_to_use = f"{name_prefix}{random.choice(string.ascii_lowercase)}-{random.randint(1, max_val)}"
    ...
    while name_to_use in collection_dict and attempts < attempts_limit:
        if suffix_chaos:
            ...
            name_to_use = f"{name_prefix}{suffix_format.format(random.randint(1, max_val))}"
        attempts += 1
    if attempts >= attempts_limit and name_to_use in collection_dict : # Still a collision
        name_to_use = f"{name_to_use}-{uuid.uuid4().hex[:4]}" # Add short UUID to force uniqueness
...
# These functions create entities, store them in SHARED_CONTEXT, and return the entity dictionary.
# They rely on current_row_context to get parent entities if needed.
# `is_primary_identifier=True` means this function call is defining the entity for the first time.
# `force_new=True` can be used to bypass reuse probability for primary definitions.

def generate_datacenter_entity(current_row_context=None, is_primary_identifier=False, force_new=False, **kwargs):
    if DEBUG_ENTITY_GENERATION: print(f"DEBUG: DC Entity. Primary={is_primary_identifier}, ForceNew={force_new}, ExistingDCs={len(SHARED_CONTEXT['dcs'])}")
    if not force_new and not is_primary_identifier and SHARED_CONTEXT['dcs'] and random.random() < REUSE_PROBABILITY:
        return random.choice(list(SHARED_CONTEXT['dcs'].values()))
    ...
    entity = {"id": f"dc-{uuid.uuid4().hex[:8]}", "name": name}
    SHARED_CONTEXT['dcs'][name] = entity
    if DEBUG_ENTITY_GENERATION: print(f"DEBUG: Created DC: {name}")
    return entity
...
def generate_cluster_entity(current_row_context=None, is_primary_identifier=False, force_new=False, **kwargs):
    ...
    if DEBUG_ENTITY_GENERATION: print(f"DEBUG: Cluster Entity. Primary={is_primary_identifier}, ForceNew={force_new}, DC={dc_entity['name']}, ExistingClusters={len(SHARED_CONTEXT['clusters'])}")
    ...
    entity = {"id": f"cluster-{uuid.uuid4().hex[:8]}", "name": name, "dc_name": dc_entity['name']}
    SHARED_CONTEXT['clusters'][name] = entity
    if DEBUG_ENTITY_GENERATION: print(f"DEBUG: Created Cluster: {name} in DC {dc_entity['name']}")
    return entity
...
def generate_host_entity(current_row_context=None, is_primary_identifier=False, force_new=False, **kwargs):
    ...
    if DEBUG_ENTITY_GENERATION: print(f"DEBUG: Host Entity. Primary={is_primary_identifier}, ForceNew={force_new}, Cluster={cluster_entity['name']}, ExistingHosts={len(SHARED_CONTEXT['hosts'])}")
    ...
    entity = {"id": f"host-{uuid.uuid4().hex[:8]}", "name": name, "ip": generate_ip_address(),
              "cluster_name": cluster_entity['name'], "dc_name": cluster_entity['dc_name']}
    SHARED_CONTEXT['hosts'][name] = entity
    if DEBUG_ENTITY_GENERATION: print(f"DEBUG: Created Host: {name} in Cluster {cluster_entity['name']}")
    return entity
...
def generate_vm_entity(current_row_context=None, is_primary_identifier=False, force_new=False, **kwargs):
    ...
    if DEBUG_ENTITY_GENERATION: print(f"DEBUG: VM Entity. Primary={is_primary_identifier}, ForceNew={force_new}, Host={host_entity['name']}, ExistingVMs={len(SHARED_CONTEXT['vms'])}")
    ...
    entity = {"id": f"vm-{uuid.uuid4().hex[:12]}", "name": name, "ip": generate_ip_address(), "mac": generate_mac_address(),
              "host_name": host_entity['name'], "powerstate": generate_power_state(), "os": generate_os_name(),
              "cluster_name": host_entity['cluster_name'], "dc_name": host_entity['dc_name']}
    SHARED_CONTEXT['vms'][name] = entity
    if DEBUG_ENTITY_GENERATION: print(f"DEBUG: Created VM: {name} on Host {host_entity['name']}")
    return entity

def generate_datastore_entity(current_row_context=None, is_primary_identifier=False, force_new=False, **kwargs):
    if DEBUG_ENTITY_GENERATION: print(f"DEBUG: Datastore Entity. Primary={is_primary_identifier}, ForceNew={force_new}, Existing={len(SHARED_CONTEXT['datastores'])}")
    if not force_new and not is_primary_identifier and SHARED_CONTEXT['datastores'] and random.random() < REUSE_PROBABILITY:
        return random.choice(list(SHARED_CONTEXT['datastores'].values()))
    name_prefix = f"ds-{random.choice(['local', 'san', 'nfs', 'iscsi'])}-{random.choice(['ssd', 'hdd', 'vmfs', 'nvme'])}-"
    name = _ensure_unique_name(name_prefix, SHARED_CONTEXT['datastores'], 20, MAX_NAME_GEN_ATTEMPTS)
    entity = {"id": f"ds-{uuid.uuid4().hex[:8]}", "name": name, "type": name.split('-')[1]}
    SHARED_CONTEXT['datastores'][name] = entity
    return entity

def generate_network_entity(current_row_context=None, is_primary_identifier=False, force_new=False, **kwargs):
    if DEBUG_ENTITY_GENERATION: print(f"DEBUG: Network Entity. Primary={is_primary_identifier}, ForceNew={force_new}, Existing={len(SHARED_CONTEXT['networks'])}")
    if not force_new and not is_primary_identifier and SHARED_CONTEXT['networks'] and random.random() < REUSE_PROBABILITY:
        return random.choice(list(SHARED_CONTEXT['networks'].values()))
    name_prefix = f"{random.choice(['Prod-Net', 'VLAN', 'Dev-Seg', 'DMZ-Net'])}-"
    name = _ensure_unique_name(name_prefix, SHARED_CONTEXT['networks'], 500, MAX_NAME_GEN_ATTEMPTS, suffix_format="{}")
    entity = {"id": f"net-{uuid.uuid4().hex[:8]}", "name": name}
    SHARED_CONTEXT['networks'][name] = entity
    return entity

# --- CSV File Generation ---
def generate_csv_file(csv_filename_to_generate, headers, num_rows_to_generate, column_strategies):
    """
    Generates a CSV file based on defined strategies for each column.
    - csv_filename_to_generate: Name of the CSV file.
    - headers: List of column headers.
    - num_rows_to_generate: Number of data rows.
    - column_strategies: A dictionary mapping column headers to generation strategies.
      Each strategy defines:
        - "generator": The function to call.
        - "args": A dict of arguments for the generator (optional).
        - "is_entity_generator": True if the generator returns a whole entity dict (optional).
        - "attribute_to_extract": Key of the attribute to get from the entity dict for the cell (if is_entity_generator).
        - "entity_context_key": Key under which to store the generated entity in current_row_context.
        - "source": "context_entity_attribute" to pull from an entity already in current_row_context.
        - "entity_context_to_source_from": Key of the entity in current_row_context.
        - "attribute": Attribute name to get from the sourced entity.
        - "min_col_val_from_context", "max_col_val_from_context": For ranged integers based on other columns in the row.
    """
    if DEBUG_ENTITY_GENERATION: print(f"\n--- Generating CSV: {csv_filename_to_generate} ---")
    try:
        with open(csv_filename_to_generate, 'w', newline='') as csvfile:
            ...
            for i in range(num_rows_to_generate):
                ...
                for header in headers:
                    ...
                    if strategy:
                        ...
                        args_to_pass = {**args}
                        is_entity_gen = strategy.get("is_entity_generator", False)
                        attribute_to_extract = strategy.get("attribute_to_extract")
                        entity_context_key = strategy.get("entity_context_key")
                        ...
                        if generator_func == generate_ai_data and "prompt" not in cleaned_args_to_pass: cleaned_args_to_pass["prompt"] = f"Value for {header}"

                        raw_generated_value = generator_func(**cleaned_args_to_pass)

                        if is_entity_gen and entity_context_key :
                            current_row_context[entity_context_key] = raw_generated_value
                            generated_value = raw_generated_value.get(attribute_to_extract, "")
                        elif strategy.get("source") == "context_entity_attribute":
                            source_key = strategy.get("entity_context_to_source_from")
                            attr_key = strategy.get("attribute")
                            entity_in_context = current_row_context.get(source_key)
                            ...
                            generated_value = raw_generated_value
                    else:
                        generated_value = generate_random_string(length=random.randint(5,15))
                    row_data.append(str(generated_value))
                    current_row_context[header] = generated_value # Store actual cell value
                writer.writerow(row_data)
        print(f"Successfully generated {num_rows_to_generate} rows into {csv_filename_to_generate}")
    except Exception as e:
        import traceback
        print(f"ERROR generating {csv_filename_to_generate} (Header: {header if 'header' in locals() else 'N/A'}, Row: {i+1 if 'i' in locals() else 'N/A'}): {e}")
        traceback.print_exc()

# --- Zip Generated Files ---
def zip_generated_csvs(output_zip_filename="RVTools_synthetic_data.zip", generated_files_prefix="generated_"):
    """Zips all CSV files starting with the specified prefix."""
    try:
        generated_csv_files = [f for f in os.listdir('.') if f.startswith(generated_files_prefix) and f.endswith('.csv')]
        if not generated_csv_files:
            print("No generated CSV files found to zip.")
            return

        with zipfile.ZipFile(output_zip_filename, 'w', zipfile.ZIP_DEFLATED) as zipf:
            for file_to_zip in generated_csv_files:
                zipf.write(file_to_zip)
                os.remove(file_to_zip) # Optionally remove individual CSVs after zipping
        print(f"Successfully zipped {len(generated_csv_files)} files into {output_zip_filename}")
    except Exception as e:
...
# --- Main Function ---
def main():
    print("Starting RVTools Synthetic Data Generator (Blueprint Version)...\n")
    # Note: num_rows_per_csv can be increased for more substantial data generation locally.
    # For restricted environments, smaller numbers are safer to avoid timeouts.
    num_rows_per_csv = 15 # Default for blueprint

    # --- Pre-generate some initial entities to ensure SHARED_CONTEXT is not empty ---
    # This helps avoid issues where the first few CSVs might not have parent entities to pick from.
    if not SHARED_CONTEXT['dcs']:
        for _ in range(2): generate_datacenter_entity(force_new=True) # Create 2 initial DCs
    if SHARED_CONTEXT['dcs'] and not SHARED_CONTEXT['clusters']:
        for dc_name in list(SHARED_CONTEXT['dcs'].keys())[:1]: # For the first DC
             for _ in range(1): generate_cluster_entity(current_row_context={'current_dc_entity': SHARED_CONTEXT['dcs'][dc_name]}, force_new=True) # Create 1 cluster in it
    if SHARED_CONTEXT['clusters'] and not SHARED_CONTEXT['hosts']:
        for cl_name in list(SHARED_CONTEXT['clusters'].keys())[:1]: # For the first Cluster
            for _ in range(2): generate_host_entity(current_row_context={'current_cluster_entity': SHARED_CONTEXT['clusters'][cl_name]}, force_new=True) # Create 2 hosts in it

    for csv_filename_key in CSV_GENERATION_ORDER:
        if csv_filename_key not in CSV_HEADERS:
            print(f"Warning: {csv_filename_key} in generation order but not in CSV_HEADERS. Skipping.")
            continue

        headers_list = CSV_HEADERS[csv_filename_key]
        column_strategies = {}

        # Context flags for determining primary entity definition points
        is_defining_dcs = "tabvhost" in csv_filename_key.lower() # Assume DCs primarily defined via Hosts
        is_defining_clusters = "tabvhost" in csv_filename_key.lower() # Clusters via Hosts
        ...
        # General strategy application loop
        for header in headers_list:
            header_lower = header.lower()
            # Default: random string. Strategies below will override.
            column_strategies[header] = {"generator": generate_random_string, "args": {"length": random.randint(6, 12)}}

            # --- Core Entities & Linking ---
            if header == "Datacenter":
                column_strategies[header] = {"generator": generate_datacenter_entity, "args": {"is_primary_identifier": is_defining_dcs, "force_new": is_defining_dcs and len(SHARED_CONTEXT['dcs']) < num_rows_per_csv * 0.2}, "is_entity_generator": True, "attribute_to_extract": "name", "entity_context_key": "current_dc_entity"}
            elif header == "Cluster":
                column_strategies[header] = {"generator": generate_cluster_entity, "args": {"is_primary_identifier": is_defining_clusters, "force_new": is_defining_clusters and len(SHARED_CONTEXT['clusters']) < num_rows_per_csv * 0.5}, "is_entity_generator": True, "attribute_to_extract": "name", "entity_context_key": "current_cluster_entity"}
            elif header == "ESX Host":
                column_strategies[header] = {"generator": generate_host_entity, "args": {"is_primary_identifier": is_defining_hosts, "force_new": is_defining_hosts and len(SHARED_CONTEXT['hosts']) < num_rows_per_csv}, "is_entity_generator": True, "attribute_to_extract": "name", "entity_context_key": "current_host_entity"}

            elif header == "DNS Name" and is_defining_vms: # Primary definition of a VM
                column_strategies[header] = {"generator": generate_vm_entity, "args": {"is_primary_identifier": True, "force_new":True}, "is_entity_generator": True, "attribute_to_extract": "name", "entity_context_key": "current_vm_entity"}
            elif header == "DNS Name": # Referencing a VM (e.g. in vDisk, vNetwork)
                 column_strategies[header] = {"generator": generate_vm_entity, "args": {"is_primary_identifier": False}, "is_entity_generator": True, "attribute_to_extract": "name", "entity_context_key": "current_vm_entity"}

            # --- Attribute Sourcing from Context ---
            elif header == "VM ID":
                 column_strategies[header] = {"source": "context_entity_attribute", "entity_context_to_source_from": "current_vm_entity", "attribute": "id", "fallback_generator": generate_random_string, "fallback_args":{"length":12}}
            elif header == "IP Address":
                if "vm" in csv_filename_key.lower() or "nic" in csv_filename_key.lower() and "host" not in csv_filename_key.lower() : # VM's IP
                     column_strategies[header] = {"source": "context_entity_attribute", "entity_context_to_source_from": "current_vm_entity", "attribute": "ip", "fallback_generator": generate_ip_address}
                elif "host" in csv_filename_key.lower() and "nic" in csv_filename_key.lower(): # Host's IP (from vNIC)
                     column_strategies[header] = {"source": "context_entity_attribute", "entity_context_to_source_from": "current_host_entity", "attribute": "ip", "fallback_generator": generate_ip_address}
                else: column_strategies[header] = {"generator": generate_ip_address}
            elif header == "MAC Address":
                 column_strategies[header] = {"source": "context_entity_attribute", "entity_context_to_source_from": "current_vm_entity", "attribute": "mac", "fallback_generator": generate_mac_address}
            elif header_lower == "powerstate":
                column_strategies[header] = {"source": "context_entity_attribute", "entity_context_to_source_from": "current_vm_entity", "attribute": "powerstate", "fallback_generator": generate_power_state}
            elif "os according to the vmware tools" in header_lower or "os according to the configuration file" in header_lower:
                column_strategies[header] = {"source": "context_entity_attribute", "entity_context_to_source_from": "current_vm_entity", "attribute": "os", "fallback_generator": generate_os_name}

            # Datastores & Networks
            elif header == "Datastore" or "datastore name" in header_lower:
                 column_strategies[header] = {"generator": generate_datastore_entity, "args": {"is_primary_identifier": is_defining_datastores, "force_new": is_defining_datastores and len(SHARED_CONTEXT['datastores']) < num_rows_per_csv * 0.3}, "is_entity_generator": True, "attribute_to_extract": "name", "entity_context_key": "current_datastore_entity"}
            elif "network adapter" in header_lower or "port group" in header_lower or "switch name" in header_lower or (header_lower.startswith("network #") and is_defining_vms): # Network #N in vInfo
                 column_strategies[header] = {"generator": generate_network_entity, "args": {"is_primary_identifier": is_defining_networks, "force_new": is_defining_networks and len(SHARED_CONTEXT['networks']) < num_rows_per_csv * 0.5}, "is_entity_generator": True, "attribute_to_extract": "name", "entity_context_key": "current_network_entity"}

            # Standard generators for common fields
            ...
            elif "uuid" in header_lower: column_strategies[header] = {"generator": generate_random_string, "args": {"length":36}} # UUID format
        ...
        output_filename = f"generated_{csv_filename_key}"
        generate_csv_file(output_filename, headers_list, num_rows_per_csv, column_strategies)

    zip_generated_csvs() # Zip all generated files
